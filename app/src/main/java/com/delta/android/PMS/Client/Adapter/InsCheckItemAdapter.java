...
public class InsCheckItemAdapter extends BaseAdapter {
    ...
    public InsCheckItemAdapter(FragmentActivity context, LayoutInflater inflater, Cursor data, String woid) {
        mInflater = inflater;
        conText = context;
        woId = woid;
        if (data.getCount() != 0) {
...
//        tvStandardWorkingHour.setText(arData.get(position).get("STD_HOUR"));
        ...
        btnPMINFO.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                final WorkInsActivity activity = (WorkInsActivity) conText;
                if (!activity.GetIsWork()) {
                    ((WorkInsActivity) conText).ShowMessage(conText.getResources().getString(R.string.EAPE105020));
                    return;
                }
