...
public class BGoodReceiptReceiveParam extends BStockBaseParam {

    public static final String TrxType = "Unicom.Uniworks.BModule.WMS.INV.Parameter.GoodReceiptReceiveParam.TrxType";

    public static final String GrrMasterObj = "Unicom.Uniworks.BModule.WMS.INV.Parameter.GoodReceiptReceiveParam.GrrMasterObj";

    public  class GrrMasterObj
    {
        private String GrID ;
        private ArrayList<GrrDetObj> GrDetails ;
        ...
        public ArrayList<GrrDetObj> getGrDetails() {
            return GrDetails;
        }
        public void setGrDetails(ArrayList<GrrDetObj>  grDetails) {
            GrDetails = grDetails;
        }
        ...
        public final GrrMasterObj GetGrrSheet(DataTable dtMst, DataTable dtDet, DataTable dtSn)
        {
            GrrMasterObj mst = new GrrMasterObj();
            GrrDetObj det = null;
            //if (dtMst.getColumns().contains("GR_ID"))
                mst.setGrID(dtMst.Rows.get(0).getValue("GR_ID").toString());
            ArrayList<GrrDetObj> detList = new ArrayList<>();
            for (DataRow dr : dtDet.Rows)
            {
                det = new GrrDetObj();
                //if (dtDet.getColumns().contains("SEQ"))
                    det.setSeq("0"+dr.getValue("SEQ").toString());
                //if (dtDet.getColumns().contains("STORAGE_ID"))
                    det.setStorageId(dr.getValue("STORAGE_ID").toString());
                //if (dtDet.getColumns().contains("ITEM_ID"))
                    det.setItemId(dr.getValue("ITEM_ID").toString());
                //if (dtDet.getColumns().contains("LOT_ID"))
                    det.setLotId(dr.getValue("LOT_ID").toString());
                //if (dtDet.getColumns().contains("QTY"))
                    det.setQty("0"+dr.getValue("QTY").toString());
                //if (dtDet.getColumns().contains("UOM"))
                    det.setUom(dr.getValue("UOM").toString());
                //if (dtDet.getColumns().contains("CMT"))
                ...
                //if (dtDet.getColumns().contains("MFG_DATE"))
                {
                    if (!(dr.getValue("MFG_DATE").toString().equals("")))
                        det.setMfgDate(dr.getValue("MFG_DATE").toString().substring(0,10) + " 00:00:00");
                }
                //if (dtDet.getColumns().contains("EXP_DATE"))
                {
                    if (!(dr.getValue("EXP_DATE").toString().equals("")))
                        det.setExpDate(dr.getValue("EXP_DATE").toString().substring(0,10) + " 23:59:59");
                }
                /*{
                    if (!(dr.getValue("PO").toString().equals("")))
                        det.setPo(dr.getValue("PO").toString());
                }
                {
                    if (!(dr.getValue("PO_SEQ").toString().equals("")))
                        det.setPoSeq(dr.getValue("PO_SEQ").toString());
                }*/
                {
                ...
                {
                    ArrayList<GrrSnObj> lstSn = new ArrayList<GrrSnObj>();
    ...
    public class GrrDetObj
    {
        ...
        public ArrayList<GrrSnObj> getGrSns() {
            return GrSns;
        }
